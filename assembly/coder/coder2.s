...
main:
    ...
    jnz NUM_ARGS_BAD        # if numbers of args is not 5, then display usage message and get out
...
# Check command line switch for '-d' or '-e' for decode/encode
...
GET_XOR:
    # Get argv[2] which is our xor byte
    mov rdi, [rbp - ARGV]           # retrieve *argv[]

    mov rdi, [rdi + 16]              # argv[1]
    ...
    cmp rax, 0x00               # Make sure out value is one byte
    jl HEX_INVALID
...
CODER_LOOP:
    ...
    mov al, [rbp - BUFFER]      # put the read character into al
    ...
    xor al, [rbp - XOR_VAL]     # xor encode with the provided value on command line

    mov [rbp - BUFFER], al      # save to buffer to get ready to write to outfile

    jmp WRITE                   # Decode done, jump to write the byte

# If bx is 0, then do encode and write bit immediately after
DO_ENCODE:
...
.section .rodata
    USAGE: .ascii "Usage: coder <-d|-e> <xor byte> <infile> <outfile>\n"
    .set USAGE_LEN, . - USAGE
